...
class PPOAgent:
    ...
    def __init__(self, numberOfInputs, numberOfActorOutputs, a_params, clip_param=0.2, max_grad_norm=0.5, ppo_update_iters=5,
        ...
        # models
        self.actor_net = Actor(numberOfInputs, numberOfActorOutputs, a_params)
...
class Actor(nn.Module):
    def __init__(self, numberOfInputs, numberOfOutputs, a_params):
        super(Actor, self).__init__()
        a_params0 = np.asarray(a_params[0]).reshape((64, numberOfInputs + 1))
        a_params1 = np.asarray(a_params[1]).reshape((64, 64 + 1))
        a_params2 = np.asarray(a_params[2]).reshape((numberOfOutputs, 64+1))
        self.fc1 = nn.Linear(numberOfInputs, 64)
        weight0 = torch.Tensor(a_params0[:, 0:numberOfInputs])
        bias0 = torch.Tensor(a_params0[:, numberOfOutputs])
        self.fc1.weight = nn.Parameter(weight0)
        self.fc1.bias = nn.Parameter(bias0)
        self.fc2 = nn.Linear(64, 64)
        weight1 = torch.Tensor(a_params1[:, 0:64])
        bias1 = torch.Tensor(a_params1[:, 64])
        self.fc2.weight = nn.Parameter(weight1)
        self.fc2.bias = nn.Parameter(bias1)
        self.action_head = nn.Linear(64, numberOfOutputs)
        weight2 = torch.Tensor(a_params2[:, 0:64])
        bias2 = torch.Tensor(a_params2[:, 64])
        self.action_head.weight = nn.Parameter(weight2)
        self.action_head.bias = nn.Parameter(bias2)
